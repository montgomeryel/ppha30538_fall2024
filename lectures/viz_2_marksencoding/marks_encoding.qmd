...
*are two things in the same category or not*
...
* When visualizing ordinal data, we should perceive a sense of rank-order. Position, size, or color value (brightness) might be appropriate.
    * Remark: color hue (which is not perceptually ordered) would be less appropriate. *but saturation of a single color would work*
...
* In the dataset above:
    * `year` is a **quantitative interval field**: depending on whose history of the world you prefer, there are many choices for the year "zero" (there isn't really an need to plot down to zero)
