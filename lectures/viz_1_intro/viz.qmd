...
* then build our first aggregated chart
  *a chart includes data, mark, and encoding*
...
#this is a one axis version of a graph
```
...
#add x axis to code
#mark is left empty of any customizations
```
...
#using cars dataset, line graph, encoding: x and y axis
```
...
#vertical bar is a pipe symbol. two plots together
#+ puts charts on top of one another, | pipes and puts charts next to one another
```
...
print(chart.to_json())
#json output shows missing data. You can back out the data from a chart using the json keys
```
